...
function sb_options() {
	...
	if (isset($_POST['resetdefault'])) {
		...
		sb_update_option('podcast_url', sb_get_podcast_url());
	...
	elseif (isset($_POST['save'])) {
		...
		if (isset($_POST['allow_comments']) != (sb_get_option('allow_comments'))) {
			sb_update_option ('allow_comments', isset($_POST['allow_comments']));
			if (sb_get_option('display_method') != 'dynamic' && $_POST['display_method'] != 'dynamic')
				sb_set_comments (isset($_POST['allow_comments']));
		}
        if ($_POST['display_method'] != ($old_method = sb_get_option('display_method'))) {
 			sb_update_option('display_method', $_POST['display_method']);
        	if ($_POST['display_method'] == 'dynamic')
        		sb_delete_all_custom_posts();
        	elseif ($old_method == 'dynamic')
        		sb_create_all_custom_posts();
		}
		if (!is_dir(SB_ABSPATH.$dir))
			if (sb_mkdir(SB_ABSPATH.$dir))
				@chmod(SB_ABSPATH.$dir, 0777);
...
?>
	<div class="wrap">
		...
		<table border="0" class="widefat">
			...
			?>
			<tr>
				<th scope="row" style="text-align:right"><?php _e('Upload folder', $sermon_domain) ?>: </th>
			...
			?>
			<tr>
				<th scope="row" style="text-align:right"><?php _e('Public podcast feed', $sermon_domain) ?>: </th>
				<td><input type="text" name="podcast" value="<?php echo htmlspecialchars(sb_get_option('podcast_url')) ?>" style="width:100%" /></td>
			</tr>
			<tr>
				<th scope="row" style="text-align:right"><?php _e('Private podcast feed', $sermon_domain) ?>: </th>
				<td><?php echo htmlspecialchars(sb_get_podcast_url()); ?></td>
			</tr>
			<tr>
				<th scope="row" style="text-align:right"><?php _e('MP3 shortcode', $sermon_domain) ?>: </th>
				<td><input type="text" name="mp3_shortcode" value="<?php echo htmlspecialchars(sb_get_option('mp3_shortcode')) ?>" style="width:100%" /><br />
				<?php _e('The default is [audio:%SERMONURL%]', $sermon_domain); ?></td>
			</tr>
			<tr>
				<th scope="row" style="text-align:right"><?php _e('Sermons per page', $sermon_domain) ?>: </th>
				<td><input type="text" name="perpage" value="<?php echo sb_get_option('sermons_per_page') ?>" /></td>
			</tr>
			<tr>
				<th scope="row" style="text-align:right;vertical-align:top" rowspan="2"><?php _e('Filter type', $sermon_domain) ?>: </td>
				<td>
				<?php
            ...
            <?php if (sb_supports_custom_posts()) { ?>
            <tr>
                <th scope="row" style="text-align:right;vertical-align:top" rowspan="2"><?php _e('Display method', $sermon_domain) ?>:<br />
                	<span style="font-weight:normal;font-style:italic">(<?php _e('To understand the differences<br />between these options, click<br /><b>Help</b> at the top right of this page.', $sermon_domain) ?>)</span>
				</th>
                <td>
				<?php
					$dm = sb_get_option('display_method');
					$filter_options = array ('dynamic' => __('Single dynamic page', $sermon_domain), 'post' => __('A post for each sermon', $sermon_domain), 'page' => __('A page for each sermon', $sermon_domain));
					foreach ($filter_options as $value => $filter_option) {
						echo "<input ".($value == 'dynamic' ? "onclick=\"document.getElementById('allow_comments').disabled=true\"" : "onclick=\"document.getElementById('allow_comments').disabled=false\"")." type=\"radio\" name=\"display_method\" value=\"{$value}\" ";
						if ($dm == $value)
							echo 'checked="checked" ';
						echo "/> {$filter_option}<br/>\n";
					}
			...
			<tr>
				<td><input id="allow_comments" type="checkbox" <?php echo ($dm == 'dynamic' ? 'disabled="disabled"' : '') ?> name="allow_comments" <?php if (sb_get_option('allow_comments')) echo 'checked="checked" '?> value="1" \> <?php _e('Allow users to comment on sermons?', $sermon_domain) ?></td>
			</tr>
            <?php } ?>
			<tr>
				<th scope="row" style="text-align:right"><?php _e('Hide sermons without attachments?', $sermon_domain) ?></th>
				<td style="vertical-align: middle;"><input type="checkbox" name="hide_no_attachments" <?php if (sb_get_option('hide_no_attachments')) echo 'checked="checked" '?> value="1" \></td>
			</tr>
			<?php
			...
			<table class="widefat">
				...
				<tr>
					<td>
						...
						<div><?php sb_print_preacher_image($preacher, true, array ('width' => 250, 'height' => 250))?></div>
			...
			<tbody>
				<?php foreach ((array) $preachers as $preacher): ?>
					<tr class="<?php $i=0; echo (++$i % 2 == 0) ? 'alternate' : '' ?>">
						<td><?php echo ($preacher->image == '') ? '' : sb_print_preacher_image($preacher, true, array ('width' => 75, 'height' => 75))?></td>
						<td style="vertical-align: middle;"><?php echo $preacher->id ?></td>
						<td style="vertical-align: middle;"><?php echo stripslashes($preacher->name) ?></td>
						<td style="vertical-align: middle;"><?php echo $preacher->sermon_count ?></td>
...
?>
	...
	<?php
		if (sb_checkSermonUploadable() == 'writeable') {
		?>
		<div class="wrap">
...
function sb_manage_sermons() {
	...
	if (isset($_GET['mid'])) {
		...
		do_action ('sb_delete_sermon', $mid);
				...
				<?php if (is_array($sermons)): ?>
					...
					<tr class="<?php $i=0; echo ++$i % 2 == 0 ? 'alternate' : '' ?>">
						...
						<td style="text-align:center">
							...
							<?php } ?>
							<a href="<?php echo sb_get_sermon_url($sermon->id); ?>">View</a>
						</td>
					</tr>
					<?php endforeach ?>
				<?php endif ?>
...
function sb_do_alerts() {
	global $wpdb, $sermon_domain;
	if (sb_get_option('mp3_shortcode') == '')
		sb_update_option ('mp3_shortcode', '%SERMONURL%');
		...
		if ((substr(sb_get_option('mp3_shortcode'), 0, 18) == '[audio:%SERMONURL%') && !function_exists('ap_insert_player_widgets')) {
			if ($wpdb->get_var("SELECT COUNT(id) FROM {$wpdb->prefix}sb_stuff WHERE name LIKE '%.mp3'")>0) {
				...
				if (version_compare($wp_version, '2.7', '<'))
					$wpaurl = 'http://wordpress.org/extend/plugins/audio-player/';
				else
					$wpaurl = site_url().'/wp-admin/plugin-install.php?tab=search&s=audio%20player%20doryphores&type=term&search=Search';
				echo '<div id="message" class="updated"><p><b>'.sprintf(__('Tip: Installing the %1$sWordPress Audio Player%2$s, or another WordPress MP3 player plugin will allow users to listen to your sermons more easily.', $sermon_domain), "<a href=\"{$wpaurl}\">", '</a>').'</b></div>';
			}
		} elseif (substr(sb_get_option('mp3_shortcode'), 0, 18) != '[audio:%SERMONURL%' && sb_get_option('mp3_shortcode') != '%SERMONURL%') {
			echo '<div id="message" class="updated fade"><p><b>';
...
function sb_rightnow () {
	...
	if ($file_count > 0) {
		...
		if ($download_count > 1) {
			...
			$most_popular_title = '<a href="'.sb_get_sermon_url($most_popular->sermon_id).'">'.stripslashes($most_popular->title).'</a>';
...
/**
* Adds the contextual help in admin pages (filter)
*
* @param string $help - the existing help text
* @returns string - the new help text
*/
	...
	if (!isset($_GET['page']))
		...
		switch ($_GET['page']) {
			...
			case 'sermon-browser/options.php':
				...
				$out .= '<li>'.__('The MP3 shortcode you need will be in the documation of your favourite MP3 plugin. Use the tag %SERMONURL% in place of the URL of the MP3 file (e.g. [haiku url="%SERMONURL%"] or [audio:%SERMONURL%]).', $sermon_domain).'</li>';
				$out .= '<li>'.__('If you are using WordPress 2.9 and above and are not using the default permalinks, there are also three different display methods to choose from. These affect the style of your display (the exact difference depends on your theme), the type of link used for each sermon, and whether comments can be allowed.', $sermon_domain).'<ul>';
				$out .= '<li>'.sprintf(__('%1$s is the simplest. The style will be the style of whatever post/page you have placed [sermons] on. The URL will look like this: %2$s. Comments are not allowed.', $sermon_domain), '<strong>'.__('Single dynamic page', $sermon_domain).'</strong>', '<a href="#">'.__('www.sermons.com/?sermon_id=53', $sermon_domain).'</a>').'</li>';
				$out .= '<li>'.sprintf(__('%1$s will be styled like a blog post. The URL will look like this: %2$s. Comments can be allowed if you wish, and should be displayed by all themes', $sermon_domain), ('<strong>'.__('A post for each sermon', $sermon_domain).'</strong>'), ('<a href="#">'.__('www.sermons.com/sermons/jesus-washes-the-disciples-feet', $sermon_domain).'</a>')).'</li>';
				$out .= '<li>'.sprintf(__('%1$s will be styled like a WordPress page. The URL will look like this: %2$s. Comments can be allowed if you wish, but unfortunately some themes don&#146;t show comments on pages.', $sermon_domain), '<strong>'.__('A page for each sermon', $sermon_domain).'</strong>', '<a href="#">'.__('www.sermons.com/sermons/jesus-washes-the-disciples-feet', $sermon_domain).'</a>').'</li></ul></li></ul>';
				break;
...
/**
* Deletes a custom post for a particular sermon id
*
* @param int $sermon_id
* @return bool - True on success
*/
function sb_delete_custom_post ($sermon_id) {
	global $wpdb;
	$post_id = $wpdb->get_var ("SELECT id FROM {$wpdb->prefix}posts WHERE post_type='sb_sermon' AND post_content='[sermon id={$sermon_id} type=auto]'");
	if ($post_id)
...
/**
* Deletes all custom posts
*
*/
function sb_delete_all_custom_posts ($force_delete=false) {
	global $wpdb, $wp_rewrite;
	define ('WP_IMPORTING', true);
	$post_ids = $wpdb->get_col ("SELECT id FROM {$wpdb->prefix}posts WHERE post_type='sb_sermon' AND post_content LIKE '[sermon id=% type=auto]' AND LENGTH(post_content) < 30");
	if ($post_ids)
		foreach ($post_ids as $post_id)
			if ($force_delete || !function_exists('wp_trash_post'))
				wp_delete_post($post_id);
			else
				wp_trash_post($post_id);
	sb_register_custom_post();
	$wp_rewrite->flush_rules(false);
}

/**
* (Re)creates all custom posts
*
*/
function sb_create_all_custom_posts () {
	global $wpdb, $wp_rewrite;
	define ('WP_IMPORTING', true);
	$post_ids = $wpdb->get_col ("SELECT id FROM {$wpdb->prefix}sb_sermons");
	if ($post_ids)
		foreach ($post_ids as $post_id)
			sb_update_custom_post($post_id);
	sb_register_custom_post();
	$wp_rewrite->flush_rules(false);
}

/**
* Replacement is_super_admin function
* Required for pre 3.0 compatibility
*/
function sb_is_super_admin() {
...
/**
* Globally changes the comment status of all custom posts
* 
* @var $comments_on bool - true if comments should be allowed, false if not
*/
function sb_set_comments ($comments_on) {
	global $wpdb;
	if ($comments_on)
		$wpdb->query("UPDATE {$wpdb->prefix}posts SET comment_status='open', ping_status='open' WHERE post_type='sb_sermon'");
	else
		$wpdb->query("UPDATE {$wpdb->prefix}posts SET comment_status='closed', ping_status='closed' WHERE post_type='sb_sermon'");
}
?>
