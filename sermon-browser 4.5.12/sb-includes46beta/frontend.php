...
function display_sermons($options = array()) {
	echo _deprecated_function(__FUNCTION__, '0.40', 'sb_display_sermons');
}
...
function sb_display_sermons($options = array()) {
	...
	if ($url_only == 1)
		...
		foreach ((array) $sermons as $sermon) {
			...
			if ($display_passage) {
				echo "<span class=\"sermon-passage\"> (".sb_get_books($sermon->start[0], $sermon->end[0]).")</span>";
			}
			if ($display_preacher) {
...
function sb_widget_sermon($args, $widget_args=1) {
	...
	foreach ((array) $sermons as $sermon){
		...
		echo "<a href=".sb_get_sermon_url($sermon->id).">".stripslashes($sermon->title)."</a></span>";
		if ($book) {
			if (isset ($sermon->start[0]) && isset($sermon->end[0]))
				echo " <span class=\"sermon-passage\">(".sb_get_books($sermon->start[0], $sermon->end[0]).")</span>";
		}
		if ($preacherz) {
			echo " <span class=\"sermon-preacher\">".__('by', $sermon_domain)." <a href=\"";
...
function sb_admin_bar_menu () {
	global $wp_admin_bar, $sermon_domain, $post;
	if (!current_user_can('edit_posts') || !class_exists('WP_Admin_Bar'))
		return;
	if (((isset($_GET['sermon_id']) && (int)$_GET['sermon_id'] != 0) || (isset($post) && $post->post_type == 'sb_sermon')) && current_user_can('publish_pages')) {
		if ((isset($_GET['sermon_id']) && (int)$_GET['sermon_id'] != 0))
			$sermon_id = (int)$_GET['sermon_id'];
		else
			$sermon_id = sb_get_sermon_id_from_shortcode ($post->post_content);
		$wp_admin_bar->add_menu(array('id' => 'sermon-browser-menu', 'title' => __('Edit Sermon', $sermon_domain), 'href' => admin_url('admin.php?page=sermon-browser/new_sermon.php&mid='.$sermon_id)));
	...
	if (current_user_can('manage_options')) {
		...
		$wp_admin_bar->add_menu(array('parent' => 'sermon-browser-menu', 'id' => 'sermon-browser-templates', 'title' => __('Templates', $sermon_domain), 'href' => admin_url('admin.php?page=sermon-browser/templates.php')));
	}
	...
	if ($options['display_sermons']) {
		...
		if ($sermons) {
			...
			foreach ($sermons as $sermon)
				$output['sermons'] .= '<li><a href="'.sb_get_sermon_url ($sermon->id).'">'.$sermon->title.'</a></li>';
...
function sb_print_bible_passage ($start, $end, $echo = true) {
	$output = "<p class='bible-passage'>".sb_tidy_reference($start, $end)."</p>";
	if ($echo)
		echo $output;
	else
		return $output;
...
function sb_add_bible_text ($start, $end, $version) {
	if (isset($version['version'])) {
		...
		$api = 'IP';
	if ($version == 'esv')
		return sb_add_esv_text ($start, $end, $api);
	elseif ($version == 'net')
...
//Returns ESV text
function sb_add_esv_text ($start, $end, $api = 'IP') {
	// If you are experiencing errors, you should sign up for an ESV API key
	// and specify the key in the template: [bibletext version="esv" apikey="YOURAPIKEY"]
	$esv_url = "http://www.esvapi.org/v2/rest/passageQuery?key={$api}&passage=".rawurlencode(sb_tidy_reference ($start, $end)).'&include-headings=false&include-footnotes=false';
...
function sb_add_net_text ($start, $end) {
	...
	return "<div class=\"net\">\r<h2>".sb_tidy_reference ($start, $end)."</h2><p>{$output} (<a title=\"From the NET Bible&copy; copyright &copy;1996-2006 by Biblical Studies Press, L.L.C. All rights reserved.\" href=\"http://net.bible.org/?{$reference}\">NET Bible</a>)</p></div>";
}
...
function sb_add_other_bibles ($start, $end, $version) {
	...
	if ($output)
		return '<div class="'.$version.'"><h2>'.sb_tidy_reference ($start, $end). '</h2><p>'.$output.' (<a href="http://biblepro.bibleocean.com/dox/default.aspx">'.((strlen($version) > 4) ? ucwords($version) : strtoupper($version)). '</a>)</p></div>';
}
...
function sb_build_url($arr, $clear = false) {
	...
	if ($key = array_search('podcast=1', $bar)) {
		unset($bar[$key]);
		global $wp_rewrite;
		if (isset($bar))
			if ($wp_rewrite->rewrite_rules())
				return sb_get_podcast_url().'?'.implode('&amp;', $bar);
			else
				return sb_get_podcast_url().'&amp;'.implode('&amp;', $bar);
		else
...
function sb_print_sermon_link($sermon, $echo = true) {
	if ($echo)
		echo sb_get_sermon_url($sermon->id);
	else
		return sb_get_sermon_url($sermon->id);
}

// Prints preacher search URL
function sb_print_preacher_link($sermon, $echo = true) {
	if ($echo)
		echo sb_build_url(array('preacher' => $sermon->pid));
	else
		return sb_build_url(array('preacher' => $sermon->pid));
}

// Prints series search URL
function sb_print_series_link($sermon, $echo = true) {
	if ($echo)
		echo sb_build_url(array('series' => $sermon->ssid));
	else
		return sb_build_url(array('series' => $sermon->ssid));
}

// Prints service search URL
function sb_print_service_link($sermon, $echo = true) {
	if ($echo)
		echo sb_build_url(array('service' => $sermon->sid));
	else
		return sb_build_url(array('service' => $sermon->sid));
}
...
function sb_get_tag_link($tag, $clear = false) {
	return sb_build_url(array('stag' => $tag), $clear);
}

// Prints tags
function sb_print_tags($sermon, $echo = true, $between = ', ') {
	global $wpdb;
	...
	foreach ($rawtags as $tag)
		$output[] = '<a href="'.sb_get_tag_link(stripslashes($tag->name)).'">'.stripslashes($tag->name).'</a>';
	$output = implode($between, $output);
	if ($echo)
		echo $output;
	else
		return $output;
...
//Prints tag cloud
function sb_tag_clouds($minfont=80, $maxfont=150) {
	global $wpdb;
	...
	foreach ($cnt as $tag => $count) {
		...
		$out[] = '<a style="font-size:'.(int) $size.'%" href="'.sb_get_tag_link($tag, true).'">'.$tag.'</a>';
	}
	return implode(' ', $out);
}

//Returns link to next page --changed page -> pagenum in lines 661, 663, 672, 674
function sb_next_page_link ($limit = 0) {
	...
	if ($current < ceil($record_count / $limit)) {
		$url = sb_build_url(array('pagenum' => ++$current));
		return '<a href="'.$url.'">'.__('Next page', $sermon_domain).' &raquo;</a>';
	}
}

//Returns link to previous page
function sb_prev_page_link($limit = 0) {
	global $sermon_domain;
	if ($limit == 0) $limit = sb_get_option('sermons_per_page');
	$current = isset($_REQUEST['pagenum']) ? (int) $_REQUEST['pagenum'] : 1;
	if ($current > 1) {
		$url = sb_build_url(array('pagenum' => --$current));
		return '<a href="'.$url.'">&laquo; '.__('Previous page', $sermon_domain).'</a>';
	}
}

// Print link to attached files
function sb_print_url($url, $echo = true, $atts = array('show' => 'icon', 'mp3download' => 'false')) {
	global $sermon_domain;
	...
	if (strtolower($ext) == 'mp3') {
		if ((substr(sb_get_option('mp3_shortcode'), 0, 18) == '[audio:%SERMONURL%') && function_exists('ap_insert_player_widgets'))
			$output = ap_insert_player_widgets(str_ireplace('%SERMONURL%', $url, sb_get_option('mp3_shortcode')));
		elseif (do_shortcode(sb_get_option('mp3_shortcode')) != sb_get_option('mp3_shortcode'))
			$output = do_shortcode(str_ireplace('%SERMONURL%', $url, sb_get_option('mp3_shortcode')));
		if ($atts['mp3download'] == 'true') {
			if (substr($original_url,0,7) == "http://") {
				$param="url"; }
			else {
				$param="file_name"; }
			$output .= ' <a href="'.sb_display_url().sb_query_char().'download&amp;'.$param.'='.rawurlencode($original_url).'">'.__('Download', $sermon_domain).'</a>';
		}
	}
	if (!isset($output)) {
		...
		if (in_array('icon', $atts['show']))
			$output[] = "<a href=\"{$url}\"><img class=\"site-icon\" alt=\"{$filetypes[$ext]['name']}\" title=\"{$filetypes[$ext]['name']}\" src=\"{$uicon}\"></a>";
		if (in_array('type', $atts['show']))
			$output[] = $filetypes[$ext]['name'];
		if (in_array('filename', $atts['show'])) {
			$filename = substr($original_url, strrpos($original_url, '/')+1);
			$output[] = "<a href=\"{$filename}\">".rawurlencode($filename)."</a>";
		}
...
function sb_print_url_link ($url, $echo = true) {
	$output = '<div class="sermon_file">';
	$output .= sb_print_url ($url, $echo);
	if (substr($url, -4) == ".mp3" && function_exists('ap_insert_player_widgets')) {
		...
		$output .= ' <a href="'.sb_display_url().sb_query_char().'download&amp;'.$param.'='.$url.'">Download</a>';
	}
	$output .= '</div>';
	if ($echo)
		echo $output;
...
function sb_print_code($code, $echo = true) {
	if ($echo)
...
function sb_print_preacher_description($sermon, $echo = true) {
	global $sermon_domain;
	if (strlen($sermon->preacher_description)>0) {
		$output = "<div class='preacher-description'><span class='about'>" . __('About', $sermon_domain).' '.stripslashes($sermon->preacher).': </span>';
		$output .= "<span class='description'>".stripslashes($sermon->preacher_description)."</span></div>";
		if ($echo)
			echo $output;
		else
			return $output;
...
function sb_print_next_sermon_link($sermon, $echo = true) {
	global $wpdb;
	...
	if ($next) {
		$output = '<a href="'.sb_print_sermon_link($next, false).'">'.stripslashes($next->title).' &raquo;</a>';
		if ($echo)
			echo $output;
		else
			return $output;
...
function sb_print_prev_sermon_link($sermon, $echo = true) {
	global $wpdb;
	...
	if ($prev) {
		$output = '<a href="'.sb_print_sermon_link($prev, false).'">&laquo; '.stripslashes($prev->title).'</a>';
		if ($echo)
			echo $output;
		else
			return $output;
...
function sb_print_sameday_sermon_link($sermon, $echo = true, $between = ', ') {
	global $wpdb, $sermon_domain;
	...
	if ($same) {
		...
		if ($echo)
			echo implode($output, $between);
		else
			return implode($output, $between);
	}
}
...
function sb_get_sermon_id_from_shortcode ($shortcode) {
	$shortcode = substr($shortcode, 11);
