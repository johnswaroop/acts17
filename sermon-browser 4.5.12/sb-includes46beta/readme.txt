...
4. Option to **allow users to comment** on each of your sermons.
5. **Pretty permalinks** for each sermon (e.g. www.sermons.com/sermons/jesus-walks-on-water).
6. Three optional **sidebar widgets** can display sermons on all of your posts or pages.
7. **Embed videos** and other flash files from sites such as [YouTube](http://www.youtube.com/) or [Vimeo](http://www.vimeo.com/), using either HTML code provided by those sites, or shortcode providing by a WordPress plugin.
8. **Other file types** can also be uploaded, including PDF, Powerpoint, Word, text and RTF. Multiple files can be attached to single sermons.
9. The **full Bible text** of the passage being preached on can be included on each sermon page (eight English-language versions including ESV, NASB and NET, plus dozens of other languages including Chinese, German, Korean, Portuguese, Spanish, Russian and Romanian and even NT Greek and OT Hebrew).
10. Files can be uploaded to your own site **through the browser or via FTP**. Alternatively you can use free audio hosting sites.
11. Details about each sermon can be **added automatically from the MP3's ID3 tags**.
12. Powerful **templating function** allows complete customisation to complement the look of your site.
13. **Simple statistics** show how often each sermon has been listened to.
14. Support for both **multisite** (WordPress 3.0 and above) and [WordPress MU (WPMU)](http://mu.wordpress.org/) for earlier versions of WordPress.
15. Extensive **help** and [tutorial screencasts](http://www.sermonbrowser.com/tutorials/).
16. Active [**community support fourm**](http://www.sermonbrowser.com/forum/).
17. English, German, Portugese Brazilian, Romanian, Russian and Spanish translations included.
...
Several Bible versions are supported by Sermon Browser. To switch to a different version, go to Options, and edit the single template. Replace `[bibletext version="esv"]` with the appropriate version. For example, to switch to the KJV, use the tag `[bibletext version="kjv"]`. Thanks go to Crossway for providing access to the ESV, bible.org for the NET Bible. Other versions are supplied by SermonBrowser itself. The main versions available are (but [click here for a full list](http://www.4-14.org.uk/xml-bible-web-service-api):
* **English:** akjv, amplified, asv, douayrheims, esv, kjv, nasb, net, web, ylt
* **Chinese:** chinese-ncv-simplifi, chinese-ncv-traditio, union-simplified, union-traditional
* **German:** elberfelder, elberfelder1905, luther1545, luther1912, schlachter
* **NT Greek:** byzantine, textusreceptus, tischendorf, westcott
* **OT Hebrew:** bhs, bhs-novowels
* **Korean:** korean
* **Portugese:** almeida
* **Romanian:** cornilescu
* **Russian:** synodal
* **Spanish:** rv1909, sagradas
...
Sermon Browser works out of the box, but if you wish, you can customise it to fit in with your own theme, and to display or hide whatever information you choose. If you want to create an extra page on your site that just shows a few sermons (for example, just the sermons preached at a recent conference), use **shortcodes**.
...
For ultimate control, every tag can also take attributes, like this: `[series_link exclude="None, Miscellaneous"]`. Many tags have specific attributes, but there are **two standard attributes that apply to every tag**: **before** and **after** - e.g. `[series_link before="Part of the " after=" series."]` Very simply this means print the series link, before it add the text "Part of the ", and after add the text " series.". This is very similar to writing `Part of the [series_link] series`, but the advantage of the before and after tags is that **they won't print if the tag returns a blank**. So if the series field was empty, you won't end up with `Part of the  series`.

#### Results Page ####
* **[filters_form]** - The search form which allows filtering by preacher, series, date, etc. Additional attributes:
 * `filter = "dropdown | oneclick | none"`
 * `filterhide = "show | hide"`
* **[sermons_count]** - The number of sermons which match the current search critera.
* **[sermons_loop]** - These tags should come at the beginning of the output for one sermon, and all of the tags that return data about sermons should come after this tag. **The tag must be closed with [/sermons_loop]** (see below).
* **[/sermons_loop] - Closes the sermons loop.
...
* **[podcasticon]** - Displays the icon used for the main podcast. Additional attribute:
 * `src = "http://xxx"` - the full URL of an alternative icon
* **[podcasticon_for_search]** - Displays the icon used for the custom podcast. Additional attribute:
 * `src = "http://xxx"` - the full URL of an alternative icon
* **[tag_cloud]** - Displays a tag cloud. Additional attributes:
 * `minsize = "xxx" - The minimum font size in the tag cloud, as a percentage. The default is minsize = 80.`
 * `maxsize = "xxx" - The maximum font size in the tag cloud, as a percentage. The default is maxsize = 150.`
* **[creditlink]** - displays a "Powered by Sermon Browser" link.

#### Sermon Tags ####
These tags should be used on the sermon page, or between [sermons_loop] and [/sermons_loop] on the results page.
* **[sermon_title]** - The title of the sermon
* **[preacher_link]** - The name of the preacher (hyperlinked to his search results)
* **[series_link]** - The name of the series (hyperlinked to search results). Additional attribute:
 * `exclude = "x, y"` - Don't print the name of series that are specified (you can use either the series name [case-sensitive] or the series number). For example, `[series_list exclude="None, Miscellaneous"]`. Multiple series should be separated by a comma.
 ...
 * **[file]** - Used inside the [files_loop]. Displays the files and external URLs. Additional attribute:
  * `mp3download = "true|false"` - Also add a download link to MP3 files. Useful if the audio player doesn't include its own link. Defaults to false.
  * `show = "icon|type|filename"` - Determines whether to show the icon, the filetype, or the filename. Multiple values can be separated by a comma. The default is icon.
 ...
 * **[embed]** - Used inside the [embed_loop]. Displays an embedded object (e.g. video)
* **[preacher_description]** - The description of the preacher.
* **[preacher_image]** - The photo of the preacher.
  * `width = "xxx"` - Sets the maximum width of the image to xxx pixels. Defaults to 150.
  * `height = "xxx"` - Sets the maximum height of the image to xxx pixels. Defaults to 150.
* **[passages]** - The passages associated with this sermon with the book name hyperlinked to search results. Additional attribute:
 * `between = "xxx"` - text to insert between each passage. Defaults to ", ".
* **[passages_loop][/passages_loop]** - If you want more control as to how passages are printed, you can use these two tags. Everything in between them is outputted for each passage.
 * **[passage]** - Displays the reference of a single bible passage. Designed to be used as part of the [passages_loop]. Additional attribute:
  * `between = "xxx"` - text to insert between each passage. Defaults to ", ".
 ...
 * `between = "xxx"` - text to insert between each sermon. Defaults to ", ".
* **[tags]** - Displays the tags for that sermons
* **[bibletext]** - Displays the full text for all passages linked to that sermon. Additional attribute:
 * `version = "xxx"` - the Bible version you want to use. Defaults to "esv". You can enter "none" to just display the reference, with no text, which can be useful for utilising other bible plugins (see FAQ).
* **[creditlink]** - displays a "Powered by Sermon Browser" link.
...
= 0.46 beta (20 May 2011)
* **New feature:** Allow comments on sermons (requires WordPress 2.9+). Go to options to turn it on.
* **New feature:** Allow pretty permalinks for each sermon and for the podcast feed (also requires WP 2.9+). Available when you treat each sermon as a post or page (see options).
