...
function sb_upgrade_options () {
	...
	$delete_options = array ('sb_sermon_style_output', 'sb_sermon_multi_output', 'sb_sermon_single_output');
...
function sb_database_upgrade ($old_version) {
	...
	switch ($old_version) {
		...
		case '1.1':
			...
			add_option('sb_sermon_style', sb_default_css ());
		...
		case '1.7' :
			$search_template = sb_get_option ('search_template');
			$single_template = sb_get_option ('single_template');
			$search = array ('[file_with_download]', '[first_passage]', '([passages_loop][passage][/passages_loop])', '[passages_loop][passage][/passages_loop]', '(Part of the [series_link] series).', '[esvtext]', '[kjvtext]', '[asvtext]', '[nettext]', '[ylttext]', '[webtext]', '[akjvtext]', '[hnvtext]', '[lbrvtext]', '[cornilescutext]', '[synodaltext]', '[biblepassage]', 'Part of the [series_link] series,', 'preached at a [service_link] service');
			$replace = array ('[file mp3download="true"]', '[passages]', '[passages before="(" after=")"]', '[passages]', '[series_link before="(Part of the " after=" series)." exclude="None, Misc, Miscellaneous"]', '[bibletext version="esv"]', '[bibletext version="kjv"]', '[bibletext version="asv"]', '[bibletext version="net"]', '[bibletext version="ylt"]', '[bibletext version="web"]', '[bibletext version="akjv"]', '[bibletext version="esv"]', '[bibletext version="rv1909"]', '[bibletext version="cornilescu"]', '[bibletext version="synodal"]', '[bibletext version="none"]', '[series_link before="Part of the " after=" series," exclude="None, Misc, Miscellaneous"]', '[service_link before="preached at a " after=" service"]');
			$search_template = str_replace ($search, $replace, $search_template);
			$single_template = str_replace ($search, $replace, $single_template);
			sb_update_option ('search_template', $search_template);
			sb_update_option ('single_template', $single_template);
			sb_delete_option ('search_output');
			sb_delete_option ('single_output');
			sb_update_option ('db_version', '1.8');
			return;
		default :
